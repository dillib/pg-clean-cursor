  ...
  <section style={{background:'var(--ink)', color:'var(--paper)', position:'relative', overflow:'hidden', borderBottom:'1px solid var(--paper-12)'}}>
    <div style={{maxWidth:'var(--container-w)', margin:'0 auto', padding:'120px 32px 128px', position:'relative'}}>
      ...
      <div style={{marginTop:88, display:'grid', gridTemplateColumns:'1.5fr 1fr', gap:1, background:'var(--paper-12)', border:'1px solid var(--paper-12)'}}>
        ...
        <div style={{display:'grid', gridTemplateRows:'repeat(4, 1fr)', gap:1, background:'var(--paper-12)'}}>
          {[
            ['Passports live','14,204','+1,218 this week'],
            ['P95 response','287 ms','SLA 500 ms'],
            ['Regulations','7','ESPR · Batteries · CIRPASS'],
            ['Audit exports','2,441','signed PDF · eIDAS'],
          ].map(([l,v,t]) => (
            <div key={l} style={{background:'var(--ink)', padding:'18px 22px'}}>
              <Mono style={{fontSize:10, color:'var(--paper-40)', letterSpacing:'0.1em', textTransform:'uppercase'}}>{l}</Mono>
              <div style={{fontFamily:'var(--font-display)', fontSize:32, fontWeight:600, letterSpacing:'-0.025em', color:'var(--paper)', marginTop:6, fontVariantNumeric:'tabular-nums'}}>{v}</div>
              <Mono style={{fontSize:11, color:'var(--paper-56)', marginTop:4, display:'block'}}>{t}</Mono>
            </div>
          ))}
        </div>
      </div>
...
const LogoBar = () => (
  <div style={{borderBottom:'1px solid var(--hairline)', padding:'28px 32px', background:'var(--paper)'}}>
    <div style={{maxWidth:'var(--container-w)', margin:'0 auto', display:'flex', alignItems:'center', gap:32}}>
      ...
      <div style={{display:'flex', gap:32, flex:1, justifyContent:'space-between', color:'var(--ink-72)', fontFamily:'var(--font-display)', fontWeight:600, fontSize:14, letterSpacing:'-0.01em', whiteSpace:'nowrap', overflow:'hidden'}}>
        {['NORTHFIELD','KASTNER & SÖHNE','LUMEN BATT','FOUR OAKS','HELIX ELEC.','VOLTA CELL'].map(n => <span key={n}>{n}</span>)}
      </div>
    </div>
  </div>
);
